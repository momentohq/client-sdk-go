...
func seedData(client momento.CacheClient, cacheName string) {
	for i := 0; i < 100; i++ {
		_, err := client.SetBatch(context.Background(), &momento.SetBatchRequest{
			CacheName: cacheName,
			Items:     buildBatchSetValues(i * 100),
			Ttl:       time.Hour * 1,
		})
		if err != nil {
			panic(err)
...
func buildBatchSetValues(start int) []momento.BatchSetItem {
	var returnList []momento.BatchSetItem
	for i := start; i < start+100; i++ {
		returnList = append(returnList, momento.BatchSetItem{
			Key:   momento.String(fmt.Sprintf("key%d", i)),
			Value: momento.String(strings.Repeat("x", 350)),
		})
	}
	return returnList
}
...
func buildBatchGetValues(count int) []momento.Value {
	...
	for i := 0; i < count; i++ {
		returnList = append(returnList, momento.String(
			fmt.Sprintf("key%d",
				fmt.Sprintf("key%d", rand.Intn(10000)+1)),
		),
		)
	}
	return returnList
