...
func (rh *inFlightRequestCountMiddlewareRequestHandler) OnResponse(_ interface{}) {
	countAfterRemove := rh.remover()
	rh.GetLogger().Info(
		fmt.Sprintf(
			"Request completed; in-flight requests at start: %d, in-flight requests at completion: %d",
			rh.requestsAtStart,
			countAfterRemove,
		),
	)
